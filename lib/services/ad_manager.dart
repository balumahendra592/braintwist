...
class AdManager {
  ...
  // ── Banner ──────────────────────────────────────────────────────────
  // IMPORTANT: BannerAd instances (and the AdWidget wrapping them) must
  // NOT be shared across screens. AdWidget keys itself to the exact `ad`
  // object, so reusing one static BannerAd across navigations causes
  // "This AdWidget is already in the Widget tree" crashes whenever two
  // screens briefly coexist (e.g. push/pop transitions).
  //
  // Instead, each screen that wants a banner creates its OWN BannerAd via
  // this factory, keeps it in its State, and disposes it in dispose().
